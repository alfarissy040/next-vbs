...
const AktivasiNasabahPage = () => {
    ...
    } = useSWR<extendCisUpdate[]>(`/api/cis/permintaan-ubah?${flatQueryParams(qParams)}`, fetcherNoCache, {
    ...
    const getDataUpdate = useMemo<Record<string, any>>(() => {
        ...
        const result = data.find((item: extendCisUpdate) => item.no_nas.trim().toLowerCase() === selectedUser.trim().toLowerCase());
    ...
    return (
        <section className="w-full h-full flex flex-col">
                ...
                <TableBody items={isArray(data) ? data : []} emptyContent={isError ? "Something went wrong!" : loadingData ? "" : "Data tidak ditemukan!"} isLoading={loadingData} loadingContent={<Spinner size="md" />}>
                    {(item: extendCisUpdate) => (
                        <TableRow key={item?.no_nas}>
                            ...
                            <TableCell>{item?.cis_update?.usrid_create}</TableCell>
            ...
            <Modal isOpen={isOpen} onOpenChange={handleCloseModal} backdrop="blur" classNames={CDialog} size="5xl" className="max-h-[85%] h-auto" scrollBehavior="outside">
                <ModalContent>
                    <ModalHeader className="flex flex-col gap-1">{modalState === null ? "Detail Perubahan" : "Konfirmasi Tindakan"}</ModalHeader>
                            ...
                            {modalState === null && (
                                <div className="grid grid-cols-3 gap-3">
                                    ...
                                    {getDataUpdate.map((item: cis_update) => (
                                        <CardUpdate key={item.id_update} label={item?.nm_field} currentValue={item?.current_record ?? "null"} newValue={item?.new_record ?? "null"} created_at={item?.created_at} />
                                    ))}
                                </div>
                            )}
                            {modalState === "terima" && (
                                <p>
                                    Apakah Anda yakin ingin <span className="font-semibold border-b-2 dark:border-slate-50">menerima</span> permintaan ini? Harap pastikan semua informasi sudah benar sebelum melanjutkan.
                                </p>
                            )}
                            {modalState === "tolak" && (
                                <p>
                                    Apakah Anda yakin ingin <span className="font-semibold border-b-2 dark:border-slate-50">menolak</span> permintaan ini? Harap pastikan semua informasi sudah benar sebelum melanjutkan.
                                </p>
                            )}
                        </ModalBody>
                    </ScrollShadow>
